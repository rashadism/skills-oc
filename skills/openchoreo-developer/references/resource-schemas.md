...
Prefer a matching sample from `samples/` or the live cluster schema (`get_cluster_component_type_schema`, `get_workload_schema`, `get_cluster_trait_schema`) before hand-writing a spec. Resource creation goes through MCP `create_*` / `update_*` tools — pass the spec body, not a full YAML document.
...
- The `workflow.parameters` shape is determined by the workflow's `openAPIV3Schema`. The example above is for `dockerfile-builder`; other workflows have different shapes. Always inspect with `get_cluster_workflow_schema` (or `get_workflow_schema` for namespace-scoped workflows) before authoring.
...
**Notes**:
- `ReleaseBinding` is usually created by `create_release_binding`; the spec body shown here is what flows into that call's parameters.
...
Simplified shape for understanding what platform engineers configure. Developers pick types from `list_cluster_component_types`. For full authoring detail (schema, templates, patches, validation), see `openchoreo-platform-engineer/references/component-types-and-traits.md`.
